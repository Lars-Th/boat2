...
watch(
  [() => props.startDatum, () => props.serieInställningar.antalVeckor],
  () => {
    if (props.startDatum && props.serieInställningar.antalVeckor > 1) {
      const startDate = new Date(props.startDatum);
      const endDate = new Date(startDate);
      endDate.setDate(
        startDate.getDate() + (props.serieInställningar.antalVeckor - 1) * 7
      );
  ...
  >
    ...
    <div class="flex items-center space-x-2">
      <Switch
        id="arSerie"
        :checked="arSerie"
        @update:checked="handleSerieToggle"
      />
      <Label
        for="arSerie"
    ...
    >
      ...
      <div class="col-span-full">
        ...
        >
          ...
          <span>
            Detta kommer skapa
            {{ serieInställningar.antalVeckor }} aktiviteter, en
            per {{ serieInställningar.veckodag || "vecka" }} från
            {{ startDatum || "startdatum" }}
            {{
              serieInställningar.slutDatum
                ? ` till ${serieInställningar.slutDatum}`
                : ""
            }}
          </span>
        </div>
