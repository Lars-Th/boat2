import { ref } from 'vue'
import type { ValidationRule, ValidationResult, UseValidationReturn } from '@/types'
import { customValidationRules, validateNestedProperty, type ValidationSchema } from '@/schemas/validationSchemas'
...
export function useValidation(): UseValidationReturn {
  ...
  const validateField = (fieldName: string, value: unknown, rules: string[], displayName?: string) => {
  ...
  const validateAll = (data: Record<string, unknown>, schema: Record<string, { rules: string[]; displayName?: string }>) => {
    let isValid = true
